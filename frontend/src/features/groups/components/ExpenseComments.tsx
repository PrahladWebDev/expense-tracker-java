...
export default function ExpenseComments({ groupId, expense, currentUserId, isClosed }: Props) {
  ...
  return (
    <div className="mt-2 pl-3 border-l-2 border-gray-100 dark:border-gray-800 space-y-2">
      {expense.hasReceipt && (
        <div>
          ...
          {receiptUrl && (
            <img src={receiptUrl} alt="Receipt" className="mt-2 max-h-64 rounded-md border border-gray-200 dark:border-gray-700" />
          )}
        </div>
      )}
        ...
        <div className="space-y-1.5">
          {(comments || []).map((c) => (
            <div key={c.id} className="text-xs">
              ...
              <span className="text-gray-400 dark:text-gray-500">{formatDateTime(c.createdAt)}</span>
          ...
          {(comments || []).length === 0 && <p className="text-xs text-gray-400 dark:text-gray-500">No notes yet.</p>}
        </div>
      )}
      ...
      {!isClosed && (
        <form onSubmit={onSubmit} className="flex gap-2">
          <input
            ...
            maxLength={1000}
            className="flex-1 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-brand-500"
          />
          <button
            type="submit"
